...
function rebuildScript({ profile, head }) {
  const L = [
    ...
    'else',
    '  echo "!!!! NO TEST GATE !!!!"',
    '  echo "     run-tests.js is absent from the image -- NOTHING verified this build."',
    '  echo "     The rebuild continues, but this image is being promoted untested."',
    '  NOGATE=1',
    'fi',
    'echo "=== 5. bootstrap ==="',
    'sudo -u agentadmin bash "$AD/infra/scripts/bootstrap.sh" 2>&1 | tail -20',
    'echo "=== 6. read back ==="',
    // A warning 150 lines up is not a warning. Repeat it where the operator actually looks.
    '[ "${NOGATE:-0}" = 1 ] && echo "REMINDER: this image was promoted with NO TEST GATE -- see step 4b"',
